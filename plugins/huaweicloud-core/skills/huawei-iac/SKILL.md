...
| 5. Cost + balance gate     | Price every resource via BSS `ListOnDemandResourceRatings`, then query balance | Balance = 0 → tell user pay-by-duration resources need a deposit, halt until topped up. Balance > 0 → report how long it can sustain (balance ÷ monthly estimate). Not done → deployment forbidden. Pricing codes are per-service - never guess them; see `references/resource-catalog.md` § Pricing |
