...
| Subnet DNS empty → ECS no DNS | DNS params (`--subnet.primary_dns`, `--subnet.secondary_dns`) marked optional but empty default breaks cloud-init domain resolution — `yum`/`apt` installs fail silently. Always set both. See `references/network.md` for region-specific DNS IPs |
...
| Update subnet | hcloud VPC UpdateSubnet --subnet_id=<id> --subnet.dnsList.1=<dns1> --subnet.dnsList.2=<dns2> | DNS is set via the **array param `--subnet.dnsList.N`** (1-indexed), NOT `--subnet.primary_dns`/`--subnet.secondary_dns` (those are CreateSubnet-only). Fixes a subnet created without DNS. NOTE: changing DNS does NOT re-run cloud-init user_data on already-booted instances — see huawei-ecs `references/remote-exec.md` |
| Delete subnet | hcloud VPC DeleteSubnet --vpc_id=<id> --subnet_id=<id> | Depends on `--vpc_id` + `--subnet_id`. Fails with `VPC.0209: subnet is still used` if ECS instances/nics still reference it — delete/release internal resources (instances, nics, EIPs) first, then delete the subnet |
...
| ECS cloud-init fails silently (port 80/443 closed) | Subnet likely has no DNS. Check `hcloud VPC ShowSubnet --subnet_id=<id>` → `dnsList` empty? Update subnet with `hcloud VPC UpdateSubnet --subnet_id=<id> --subnet.dnsList.1=<dns1> --subnet.dnsList.2=<dns2>` (see `references/network.md` for region-specific DNS IPs). NOTE: fixing DNS only helps NEW instances — cloud-init user_data does NOT re-run on reboot. For an instance already booted without DNS, re-create it (see huawei-ecs `references/create-instance.md` §Failure recovery) or SSH in and run the deployment manually (see huawei-ecs `references/remote-exec.md`) |
...
- Network guide (DNS by region, EIP, SG): references/network.md
